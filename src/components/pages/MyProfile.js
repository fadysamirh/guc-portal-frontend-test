...
import EditAccountModal from './ViewSalary'
...
export default function ViewAllLoc() {
  ...
  useEffect(() => {
    if (!dispatch(checkTokenExpired(history))) {
      axios({
        url: `${backendLink}/account/getprofile`,
        method: 'POST',
        headers: {
          authorization: token,
        },
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center' }}>
      ...
      <ViewSalary
        ...
        baseSalary={accounts[0] ? accounts[0].salary : 'unknown'}
      />
